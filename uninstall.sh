#!/usr/bin/env bash
# im-not-strange-ai — 전역 설치 제거 스크립트
...
if command -v gemini >/dev/null 2>&1; then
  ...
  if [ "$DRYRUN" = 1 ]; then
    echo "+ gemini extensions uninstall im-not-strange-ai (dry-run)"
  else
    gemini extensions uninstall im-not-strange-ai 2>/dev/null && echo "removed: Gemini extension (im-not-strange-ai)" \
