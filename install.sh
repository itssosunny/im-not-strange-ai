#!/usr/bin/env bash
# im-not-strange-ai — Claude Code + Codex CLI + Gemini CLI 전역 설치 스크립트
# 저장소를 클론한 뒤 `./install.sh` 한 번이면 설치된 CLI(claude/codex/gemini)를 자동 감지해
# im-not-strange-ai 스킬(+ 에이전트)을 전역으로 연결한다. 기본은 심링크(저장소 수정 즉시 반영).
set -euo pipefail
...
Usage: ./install.sh [options]
  ...
  설치된 CLI를 자동 감지해 im-not-strange-ai 스킬을 전역 설치한다.
  Claude: ~/.claude/skills/{im-not-strange-ai,im-not-strange-ai-fast,im-not-strange-ai-redo} + ~/.claude/agents/*.md
  Codex : ~/.codex/skills/im-not-strange-ai
...
if [ "$DO_GEMINI" != no ] && { [ "$DO_GEMINI" = yes ] || command -v gemini >/dev/null 2>&1; }; then
  ...
  if [ "$DRYRUN" = 1 ]; then
    ...
    echo "Y" | gemini extensions link "$REPO" 2>/dev/null && echo "installed: Gemini extension (im-not-strange-ai)" \
...
echo ""
echo "완료 (mode=$MODE)."
echo "  Claude: 새 세션에서 /im-not-strange-ai (또는 /im-not-strange-ai-fast)"
echo "  Codex : \$im-not-strange-ai"
echo "  Gemini: 새 세션에서 /im-not-strange-ai (또는 /im-not-strange-ai-fast)"
