...
description: AI(ChatGPT·Claude·Gemini)가 쓴 한글 텍스트를 사람이 쓴 글처럼 윤문한다. 번역투·영어 인용 과다·기계적 병렬·관용구·피동 남용·접속사 남발·리듬 균일·이모지/불릿 과다 등 10대 카테고리 40+ AI 티 패턴을 탐지·분류해 내용은 한 글자도 건드리지 않고 문체·리듬·표현만 자연스럽게 재작성한다. 트리거 — "AI 티 없애줘", "AI 윤문", "ChatGPT 티 제거", "번역투 고쳐", "사람이 쓴 것처럼", "im-not-strange-ai". 단순 맞춤법 교정·번역·내용 추가는 대상 아님.
---

# im-not-strange-ai — Fast Path (Codex)
...
7. **출력**: cwd 기준 `_workspace/{run_id}/final.md` 작성(run_id = `YYYY-MM-DD-NNN`, 당일 기존 폴더 있으면 NNN+1). 본문 끝에 빈 줄 하나 두고 `<!-- IM_NOT_STRANGE_AI_SUMMARY ... -->` HTML 주석 블록 1개 추가:
   - 원본/윤문본 글자수·변경률
...
- 문장 다듬기 보조 규칙: `references/sunny-sentence-rules.md` — `im-not-strange-full`의 고유 문장 점검 규칙을 중복 스킬 없이 흡수한 보조 룰북
