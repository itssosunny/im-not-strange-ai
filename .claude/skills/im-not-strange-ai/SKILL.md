...
# im-not-strange-ai — 한국어 문장 다듬기 오케스트레이터 (v0.0)

> **v0.0 현재 상태**
> `im-not-strange-ai`는 새 플러그인으로 시작하는 초기 버전입니다. Fast 모드와 strict 모드를 유지하되, 공개 identity와 Sunny 7규칙을 `im-not-strange-ai` 기준으로 정리하는 단계입니다.
>
> - **Fast 모드(디폴트)** — `im-not-strange-ai-monolith` 에이전트가 한 콜에서 탐지·윤문·자체검증 일괄 처리. 도구 호출 4~5회. 5,000자 이하 wall-clock 2~3분 목표.
> - **Strict 모드(`--strict`)** — detector·rewriter·auditor·reviewer + taxonomist 분류 자산을 분리해 정밀 검증·장문(8,000자+)을 처리합니다.
> - **Sunny 7규칙** — 문장 단위 보조 판단 기준으로 사용하되, 의미가 살아 있으면 강제로 지우지 않습니다.
...
```
im-not-strange-ai v0.0 — {fast|strict} 모드 / run_id: {YYYY-MM-DD-NNN}
```
...
### Phase 2: Monolith 호출
`im-not-strange-ai-monolith` 에이전트를 `Agent` 도구로 1회 호출.
...
5인 파이프라인으로 검증을 분리한다. 재윤문 루프가 의미 있을 때만 사용.
...
`TeamCreate`로 `im-not-strange-ai-review-team` 구성:
...
| "2차 윤문"·"`/im-not-strange-ai-redo`" | 기존 run_id의 `final.md`를 새 입력으로 strict Phase B 재실행 |
...
```
01_input.txt
    ↓ [im-not-strange-ai-monolith — 단일 호출]
...
**모델:** 모두 `model: opus` 통일. 모델보다 에이전트 내부 도구 호출 chain이 wall-clock의 핵심 병목이므로 Fast 모드는 단일 호출을 우선한다.

**에이전트 정의 위치:** 저장소 루트 `agents/`에 12종 정의(플러그인 컨벤션). Claude Code 탐색 경로:
1. 플러그인 설치 시 — `im-not-strange-ai` 플러그인이 `agents/`를 번들로 제공(전역).
2. 스크립트 설치 시 — `install.sh`가 `agents/*.md`를 `~/.claude/agents/`에 심링크(전역).

필요 에이전트 6종:
- `im-not-strange-ai-monolith` (fast 전용)
...
- 문장 다듬기 보조 규칙: [`references/sunny-sentence-rules.md`](references/sunny-sentence-rules.md) — `im-not-strange-full`의 고유 문장 점검 규칙을 중복 스킬 없이 흡수한 보조 룰북
