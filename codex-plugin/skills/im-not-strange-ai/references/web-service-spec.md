# im-not-strange-ai 웹 서비스 스펙 (Phase 5에서만 로드)

im-not-strange-ai 파이프라인을 일반 사용자용 웹앱으로 확장할 때 아키텍트가 따르는 참조. 기본 윤문 파이프라인이 안정화된 뒤에만 읽는다.
...
**경쟁 차별점**:
- 기존 영어 중심 문장 다듬기 도구(QuillBot·Hix·Undetectable AI)가 한국어에 약함.
...
```
[Browser]
    ↓ POST /api/im-not-strange-ai-fast (stream: true)
[Routing Middleware]
    ├─ BotID 검증
    ├─ 쿼터 확인 (Runtime Cache)
    └─ 언어 감지 (한글 아니면 400)
