...
### K. 문장 다듬기 보조 처방 (Sunny supplement)

AI 티 패턴을 제거한 뒤에도 문장이 어색하면
`references/sunny-sentence-rules.md`를 보조 룰북으로 읽는다. 이 처방은 새
오케스트레이터나 별도 스킬이 아니라, 윤문가가 마지막 문장 단위 점검에 쓰는
작은 보조 규칙이다.

Sunny 1~7번은 삭제 규칙이 아니라 역할 점검이다. 먼저 해당 표현이 의미,
리듬, 강조, 기술적 정밀도, 장르 격식에 필요한지 본다. 필요하면 유지하고,
역할 없이 문장을 흐리거나 술어를 미루는 경우에만 다듬는다.

| Rule | 점검 | 유지 | 다듬기 |
|---|---|---|---|
| 1 | `-적` | 개념어·전문어·대조에 필요할 때 | 추상성만 더하면 명사·동사·형용사로 환원 |
| 2 | `의` | 소유·출처·제목·고정 용어일 때 | `의`가 겹치거나 번역투가 되면 빼거나 풀어쓰기 |
| 3 | `들` | 실제 복수·분포·대조·강조일 때 | 일반명사에 붙어 뜻을 더하지 않으면 삭제 |
| 4 | `것` | 실제 대상·인용된 생각·필요한 추상화일 때 | `것이다`가 술어를 미루면 평서형으로 직결 |
| 5 | `있는/있다는` | 존재·소유·진행 상태가 핵심일 때 | 명사 앞 완충재면 삭제 |
| 6 | `있었다` | 과거 존재·위치·소유·상태일 때 | `변화가 있었다`처럼 사건 명사를 받치기만 하면 `변했다`처럼 술어 복원 |
| 7 | 어색한 `있다` 패턴 | 고정·기술 표현일 때 | `에게 있어`, `함에 있어`, `관계에 있다`가 단순 관계를 흐리면 쉬운 조사·술어로 바꿈 |

보조 처방을 적용할 때도 의미·수치·고유명사·인용·register 보존이 우선이다.
