...
`im-not-strange-ai-monolith` 에이전트가 한 콜에서 탐지·윤문·자체검증을 끝내기 위해 사용하는 슬림 룰북. 본진 `ai-tell-taxonomy.md`(590줄)에서 S1·S2 핵심 패턴만 추려 처방과 함께 한 줄로 압축했다.
...
## K. 문장 다듬기 보조 규칙 (Sunny supplement)

AI 티 패턴을 처리한 뒤에도 문장이 뻣뻣하면
`references/sunny-sentence-rules.md`를 보조 룰북으로 사용한다. 이 보조 규칙은
새 카테고리 본진이 아니라 문장 단위 마무리 점검이다. Sunny 1~7번은
삭제 규칙이 아니라 역할 점검이다. 의미·리듬·강조·정밀도·장르에 필요하면
유지하고, 역할 없이 문장만 흐리면 그때 줄인다.

| ID | 패턴 | 심각도 | 처방 |
|---|---|---|---|
| SUNNY-1 | `-적`이 붙은 말 | S2 | 개념어·전문어·대조에 필요하면 유지. 추상성만 더하면 명사·동사·형용사로 환원 |
| SUNNY-2 | 조사 `의` | S2 | 소유·출처·고정 용어면 유지. `의`가 겹치거나 번역투를 만들면 빼거나 명사구를 푼다 |
| SUNNY-3 | 접미사 `들` | S2 | 실제 복수·분포·강조면 유지. 일반명사에 붙어 의미를 더하지 않으면 뺀다 |
| SUNNY-4 | 의존명사 `것` | S2 | 실제 대상·인용된 생각·필요한 추상화면 유지. 술어를 미루는 껍데기면 직결한다 |
| SUNNY-5 | `있는/있다는` | S2 | 존재·소유·진행 상태가 핵심이면 유지. 명사 앞 완충재면 뺀다 |
| SUNNY-6 | 사족 `있었다` | S2 | 과거 존재·위치·소유·상태면 유지. 사건 명사가 이미 뜻을 담으면 동사로 복원한다 |
| SUNNY-7 | `관계에 있다`, `에게 있어`, `하는 데 있어`, `함에 있어`, `있음에 틀림없다` | S2 | 고정·기술 표현이면 유지. 단순 관계를 흐리면 쉬운 술어·조사로 바꾼다 |
