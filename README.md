# im-not-strange-ai — 한국어 문장 다듬기 플러그인 v0.0

<p align="center">
  <img src="assets/social-preview.png" alt="im-not-strange-ai — 한국어 문장 다듬기 플러그인" width="820">
</p>

`im-not-strange-ai`는 한국어 문장 다듬기 플러그인입니다. AI(ChatGPT · Claude · Gemini 등)가 쓴 한글 글을 **내용은 보존한 채** 문체 · 리듬 · 표현만 자연스러운 한국어로 다듬습니다.
...
현재는 v0.0 공개 초기 버전입니다. Claude Code와 Codex에서 marketplace source로 추가해 설치할 수 있도록 배포 메타데이터를 제공합니다.
이 방식은 self-hosted marketplace 배포입니다. Claude community/official marketplace나 Codex official Plugin Directory 등재는 별도 제출·검토 단계입니다.
...
Claude Code 안에서는 같은 작업을 `/plugin marketplace add itssosunny/im-not-strange-ai`와 `/plugin install im-not-strange-ai@im-not-strange-ai`로 실행할 수 있습니다.
...
Claude Code와 Codex CLI 모두 이 저장소를 marketplace source로 추가해 설치할 수 있습니다.
공식/커뮤니티 디렉터리 등재와는 별개로, 이 저장소를 직접 추가하는 self-hosted 배포 방식입니다.
...
- **MIT 라이선스** — [`LICENSE`](LICENSE) 참조. 코드·스킬·에이전트 정의·분류 체계 문서를 포함한 본 리포 전체에 적용됩니다.
...
다른 형태(외부 회귀 케이스 제공·슬래시 커맨드·Plugin 통합·다국어 확장 등)도 환영합니다.
